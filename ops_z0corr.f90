...
! CALLED FUNCTIONS   : stabcm
! UPDATE HISTORY     :
!-------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE ops_z0corr(z01, uster1, ol1, z02, uster2, ol2)
...
50 CONTINUE
   ...
   IF (delta_old .LT. delta .and. delta_old .NE. 0 .and. ol1 .LT. 0) THEN
      WRITE(*,'('' no convergence in subr. z0corr'')')
      WRITE(*,'(6f8.2)') z01,uster1,ol1,z02,uster2,ol2
      RETURN
   ENDIF
   delta_old=delta
   ...
   IF (ur .LE. 0) THEN
      WRITE(*,'('' negative u* in subr. z0corr'')')
      RETURN
   ENDIF
